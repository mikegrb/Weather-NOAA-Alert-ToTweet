...
my $storable_data_file = 'data';
my $seen_senders_file  = 'seen.txt';

my $data = -r $storable_data_file ? retrieve($storable_data_file) : {};
my $new_data = 0;

my $alert = Weather::NOAA::Alert->new( ['US'] );
$alert->errorLog(1);

# my $events = $alert->get_events()->{'US'};
for my $event ( @{ $alert->poll_events() } ) {
    next if ( $data->{seen_cap}{$event} );

    $data->{seen_cap}{$event} = localtime;
    $new_data = 1;    

    my $details = $alert->retrieveCAP('US', $event);
    my $sender = $details->{senderName};

    if ( !exists $data->{seen_sender}{$sender} ) {
        say "New Sender: $sender";
        open( my $fh, '>>', $seen_senders_file ) or next;
        say $fh $sender;
        close($fh) or next;
        $data->{seen_sender}{$sender} = 1;
    }

    sleep 2;
}

store $data, $storable_data_file;
